...
class PDBParser {
    parse(content) {
        ...
        const structure = new structure_1.Structure('Imported PDB');
        for (const line of lines) {
            if (line.startsWith('CRYST1')) {
                ...
                if (Number.isFinite(a) &&
                    ...
                    structure.unitCell = new unitCell_1.UnitCell(a, b, c, alpha, beta, gamma);
                }
                continue;
            }
    ...
    serialize(structure) {
        ...
        if (structure.unitCell) {
            ...
            lines.push(`CRYST1${uc.a.toFixed(3).padStart(9)}${uc.b.toFixed(3).padStart(9)}${uc.c.toFixed(3).padStart(9)}` +
        ...
        for (const atom of structure.atoms) {
            ...
            lines.push(`ATOM  ${serial}  ${name} MOL     1    ${x}${y}${z}  1.00  0.00          ${element}`);
            atomIndex++;
        }
        lines.push('END');
        return lines.join('\n');
